...
<body>
    ...
    <div id="qrcode"></div>
    <p>Status: <b style="color:#25D366">${whatsapp.status}</b></p>
...
app.post('/api/whatsapp-reset', async (req, res) => {
    try {
        // Envia comando para redefinir o serviço de WhatsApp
        // Como o serviço é independente, uma forma simples é apagar a sessão
        const authPath = path.join(__dirname, '.wwebjs_auth_hf');
        if (fs.existsSync(authPath)) fs.rmSync(authPath, { recursive: true, force: true });
        res.json({ success: true, note: 'Sessão apagada. Reinicie o serviço de WhatsApp manualmente se necessário.' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
async function ejetaScraper(limit = 50) {
    ...
    try {
        ...
        if (newResults.length > 0) {
            ...
            await queueWhatsApp(newResults);
        } else {
...
async function queueWhatsApp(data) {
    try {
        ...
        console.log(`📂 WHATSAPP: Mensagem enfileirada em ${fileName}`);
    } catch (err) {
